        ...
        return {
            ...
            problems: await Promise.all(
                problems.map((problem) =>
                    this.problemService.getProblemBaseDetailAsync(problem, query.queryTags ?? false),
                ),
            ),
        };
    }
    ...
    async getProblemDetailFilesAsync(
        ...
        return {
            files: await Promise.all(
                fileEntities.map((file) => this.problemFileService.getProblemFileDetailAsync(file)),
            ),
        };
    }
